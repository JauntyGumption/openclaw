...
<AccordionGroup>
  ...
  <Accordion title="Config and migrations">
    ...
    - Legacy heartbeat `tasks:` declarations in authored `HEARTBEAT.md` or monitor scratch migrate into automation jobs before only the converted blocks are removed. Doctor can also restore migration-owned authored content from scratch to `HEARTBEAT.md`; conflicts and concurrent changes are preserved for manual reconciliation.

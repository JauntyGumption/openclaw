...
- Heartbeat probe body is the configured heartbeat prompt (default: `Follow HEARTBEAT.md standing guidance and heartbeat monitor scratch when provided. Recurring tasks are automations; create or change their schedules with the automations tool, not heartbeat scratch. Do not infer or repeat old tasks from prior chats. If nothing needs attention, reply NO_REPLY.`). Inline directives in a heartbeat message apply as usual (but avoid changing session defaults from heartbeats).

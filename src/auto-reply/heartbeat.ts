...
const HEARTBEAT_CONTEXT_PROMPT = `Use HEARTBEAT.md standing guidance and heartbeat monitor scratch as the current heartbeat context when provided. ${HEARTBEAT_CRON_TASK_GUIDANCE}`;
/** Default prompt for heartbeat turns when config does not override it. */
export const HEARTBEAT_PROMPT = `${HEARTBEAT_CONTEXT_PROMPT} Reply ${SILENT_REPLY_TOKEN} when you choose not to send a user-visible message.`;
export const HEARTBEAT_RESPONSE_TOOL_INSTRUCTIONS =
  "Use heartbeat_respond to report the wake outcome. Set notify=true with notificationText when you choose to send a user-visible message; set notify=false when you choose not to.";
